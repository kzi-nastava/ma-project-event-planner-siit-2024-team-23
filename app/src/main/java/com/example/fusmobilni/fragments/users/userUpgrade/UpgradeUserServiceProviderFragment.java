package com.example.fusmobilni.fragments.users.userUpgrade;

import android.os.Bundle;

import androidx.annotation.NonNull;
import androidx.fragment.app.Fragment;
import androidx.lifecycle.ViewModelProvider;

import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;

import com.example.fusmobilni.R;
import com.example.fusmobilni.databinding.FragmentUpgradeUserRoleSelectionBinding;
import com.example.fusmobilni.databinding.FragmentUpgradeUserServiceProviderBinding;
import com.example.fusmobilni.interfaces.FragmentValidation;
import com.example.fusmobilni.viewModels.users.UpgradeUserViewModel;
import com.example.fusmobilni.viewModels.users.register.RegisterViewModel;

import java.util.Objects;

public class UpgradeUserServiceProviderFragment extends Fragment implements FragmentValidation {


    private FragmentUpgradeUserServiceProviderBinding _binding;
    private UpgradeUserViewModel _viewModel;

    public UpgradeUserServiceProviderFragment() {
        // Required empty public constructor
    }

    public static UpgradeUserServiceProviderFragment newInstance(String param1, String param2) {
        UpgradeUserServiceProviderFragment fragment = new UpgradeUserServiceProviderFragment();
        Bundle args = new Bundle();
        fragment.setArguments(args);
        return fragment;
    }

    @Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        if (getArguments() != null) {
        }
    }

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        // Inflate the layout for this fragment
        _binding = FragmentUpgradeUserServiceProviderBinding.inflate(inflater, container, false);
        View root = _binding.getRoot();
        _viewModel = new ViewModelProvider(requireActivity()).get(UpgradeUserViewModel.class);

        return root;
    }

    @Override
    public boolean validate() {
        String companyName = Objects.requireNonNull(_binding.companyNameInput.getEditText()).getText().toString().trim();
        String companyDescription = Objects.requireNonNull(_binding.companyDescriptionInput.getEditText()).getText().toString().trim();

        if (companyName.isEmpty()) {
            _binding.companyNameInput.setErrorEnabled(true);
            _binding.companyNameInput.setError("Company name is required");
            return false;
        } else {
            _binding.companyNameInput.setError(null);
            _binding.companyNameInput.setErrorEnabled(false);
        }

        if (companyDescription.isEmpty()) {
            _binding.companyDescriptionInput.setErrorEnabled(true);
            _binding.companyDescriptionInput.setError("Company description is required");
            return false;
        } else {
            _binding.companyDescriptionInput.setError(null);
            _binding.companyDescriptionInput.setErrorEnabled(false);
        }

        _viewModel.setCompanyName(companyName);
        _viewModel.setCompanyDescriptionName(companyDescription);

        return true;
    }
}